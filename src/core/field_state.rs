...
//! 
//! This module handles all field-related state including weather, terrain, 
//! global effects, and turn tracking for the battle system.

use crate::core::battle_format::BattlePosition;
use crate::core::instructions::{Terrain, Weather};
use crate::core::battle_state::pokemon_state::DamageInfo;
        ...
        if let Some(damage_info) = self.damaged_this_turn.get(&user_position) {
            if damage_info.attacker_position == attacker_position {
                ...
                let user_move_index = self
                    .moved_this_turn
                    .iter()
                    .slot(|&pos| pos == user_position);
                let attacker_move_index = self
                    .moved_this_turn
                    .iter()
                    .slot(|&pos| pos == attacker_position);
