    ...
    if state.weather != Weather::NONE && state.weather_turns_remaining.map_or(false, |turns| turns > 0) {
        if state.weather_turns_remaining == Some(1) {
            ...
            instruction_list.push(Instruction::ChangeWeather(
                crate::instruction::ChangeWeatherInstruction {
    ...
    if state.terrain != Terrain::NONE && state.terrain_turns_remaining.map_or(false, |turns| turns > 0) {
        if state.terrain_turns_remaining == Some(1) {
            ...
            instruction_list.push(Instruction::ChangeTerrain(
                crate::instruction::ChangeTerrainInstruction {
    ...
    if state.trick_room_active && state.trick_room_turns_remaining.map_or(false, |turns| turns > 0) {
        if state.trick_room_turns_remaining == Some(1) {
            ...
            instruction_list.push(Instruction::ToggleTrickRoom(
                crate::instruction::ToggleTrickRoomInstruction {
                    active: false,
                    duration: None,
                }
            ));
                    ...
                    PokemonStatus::TOXIC => {
                        ...
                        instruction_list.push(Instruction::ChangeVolatileStatusDuration(
                            ChangeVolatileStatusDurationInstruction {
                                target_position: position,
                                volatile_status: crate::instruction::VolatileStatus::ToxicCount,
                                duration_change: 1,
                            }
                        ));
            ...
            if let Some(pokemon) = side.get_active_pokemon_at_slot(slot) {
                ...
                for (volatile_status, &duration) in &pokemon.volatile_status_durations {
                    match volatile_status {
                        crate::instruction::VolatileStatus::SaltCure => {
                        ...
                        crate::instruction::VolatileStatus::Yawn => {
                            if duration > 1 {
                        ...
                        crate::instruction::VolatileStatus::LockedMove => {
...
fn calculate_toxic_damage(pokemon: &crate::state::Pokemon, generation: &GenerationMechanics) -> i16 {
    ...
    let toxic_count = pokemon.volatile_status_durations
        .get(&crate::instruction::VolatileStatus::ToxicCount)
        .copied()
        .unwrap_or(1) as f32;
        ...
        for (slot, (heal_amount, turns_remaining)) in &side.wish_healing {
            if *turns_remaining == 1 {
                ...
                instruction_list.push(Instruction::DecrementWish(
                    crate::instruction::DecrementWishInstruction {
                        target_position: position,
                    }
                ));
        ...
        for (slot, (attacker_position, damage_amount, turns_remaining, move_name)) in &side.future_sight_attacks {
            if *turns_remaining == 1 {
                ...
                instruction_list.push(Instruction::DecrementFutureSight(
                    crate::instruction::DecrementFutureSightInstruction {
                        target_position: position,
                    }
                ));
            } else if *turns_remaining > 1 {
    ...
    for side_ref in [crate::battle_format::SideReference::SideOne, crate::battle_format::SideReference::SideTwo] {
        ...
        for slot in 0..state.format.active_pokemon_count() {
            if let Some(pokemon) = side.get_active_pokemon_at_slot(slot) {
                ...
                if pokemon.volatile_statuses.contains(&crate::instruction::VolatileStatus::LeechSeed) {
                    let position = BattlePosition::new(side_ref, slot);
                    ...
                    if pokemon.ability.to_lowercase().replace(" ", "") != "magicguard" && pokemon.hp > 0 {
                        ...
                        let opponent_side_ref = match side_ref {
                            crate::battle_format::SideReference::SideOne => crate::battle_format::SideReference::SideTwo,
                            crate::battle_format::SideReference::SideTwo => crate::battle_format::SideReference::SideOne,
                        };
    ...
    for side_ref in [crate::battle_format::SideReference::SideOne, crate::battle_format::SideReference::SideTwo] {
        ...
        for slot in 0..state.format.active_pokemon_count() {
            if let Some(pokemon) = side.get_active_pokemon_at_slot(slot) {
                ...
                if pokemon.volatile_statuses.contains(&crate::instruction::VolatileStatus::Perish1) {
                    // Faint the Pokemon
                    instructions.push(StateInstructions::new(100.0, vec![
                        Instruction::PositionDamage(PositionDamageInstruction {
                            target_position: position,
                ...
                if let Some(item) = &pokemon.item {
                    match item.to_lowercase().replace(" ", "").as_str() {
                        ...
                        "flameorb" => {
                            ...
                            if pokemon.status == crate::instruction::PokemonStatus::NONE && pokemon.hp > 0 {
                                instructions.push(StateInstructions::new(100.0, vec![
                                    Instruction::ApplyStatus(crate::instruction::ApplyStatusInstruction {
                                        target_position: position,
                                        status: crate::instruction::PokemonStatus::BURN,
                                    })
                                ]));
                            }
                        }
                        "toxicorb" => {
                            ...
                            if pokemon.status == crate::instruction::PokemonStatus::NONE && pokemon.hp > 0 {
                                instructions.push(StateInstructions::new(100.0, vec![
                                    Instruction::ApplyStatus(crate::instruction::ApplyStatusInstruction {
                                        target_position: position,
                                        status: crate::instruction::PokemonStatus::TOXIC,
                                    }),
                                    // Initialize toxic counter
                                    Instruction::ApplyVolatileStatus(crate::instruction::ApplyVolatileStatusInstruction {
                                        target_position: position,
                                        volatile_status: crate::instruction::VolatileStatus::ToxicCount,
                                        duration: Some(1),
                                    })
            ...
            if let Some(pokemon) = side.get_active_pokemon_at_slot(slot) {
                ...
                match pokemon.ability.to_lowercase().replace(" ", "").as_str() {
                    "speedboost" => {
                        ...
                        if pokemon.hp > 0 {
                            let current_speed_boost = pokemon.stat_boosts.get(&crate::instruction::Stat::Speed).copied().unwrap_or(0);
                            if current_speed_boost < 6 {
                                let mut boosts = HashMap::new();
                                boosts.insert(crate::instruction::Stat::Speed, 1);
                                instructions.push(StateInstructions::new(100.0, vec![
                                    Instruction::BoostStats(crate::instruction::BoostStatsInstruction {
                                        target_position: position,
                                        stat_boosts: boosts,
                                    })
                                ]));
                    ...
                    "raindish" => {
                        ...
                        if pokemon.hp > 0 && pokemon.hp < pokemon.max_hp && state.weather == crate::instruction::Weather::RAIN {
                            let heal_amount = pokemon.max_hp / 16;
                            instructions.push(StateInstructions::new(100.0, vec![
                                Instruction::PositionHeal(PositionHealInstruction {
                                    target_position: position,
                    ...
                    "solarpower" => {
                        ...
                        if pokemon.hp > 0 && state.weather == crate::instruction::Weather::SUN {
                            let damage = pokemon.max_hp / 8;
                            instructions.push(StateInstructions::new(100.0, vec![
                                Instruction::PositionDamage(PositionDamageInstruction {
                                    target_position: position,
                    ...
                    "hungerswitch" => {
                        ...
                        if pokemon.hp > 0 && !pokemon.is_terastallized {
...
fn is_pokemon_grounded(pokemon: &crate::state::Pokemon) -> bool {
    ...
    if pokemon.volatile_statuses.contains(&crate::instruction::VolatileStatus::SkyDrop) {
        return false;
    }
...
fn calculate_salt_cure_damage(pokemon: &crate::state::Pokemon) -> i16 {
    // Check for Magic Guard ability (prevents all indirect damage)
    if pokemon.ability.to_lowercase().replace(" ", "") == "magicguard" {
        return 0;
    }
    ...
    match berry_name.to_lowercase().replace(" ", "").as_str() {
        ...
        "liechiberry" | "ganlonberry" | "salacberry" | "petayaberry" | "apicotberry" => {
            ...
            if pokemon.hp <= threshold && pokemon.hp > 0 {
                ...
                match berry_name.to_lowercase().replace(" ", "").as_str() {
                    "liechiberry" => { stat_boosts.insert(crate::instruction::Stat::Attack, 1); }
                    "ganlonberry" => { stat_boosts.insert(crate::instruction::Stat::Defense, 1); }
                    "salacberry" => { stat_boosts.insert(crate::instruction::Stat::Speed, 1); }
                    "petayaberry" => { stat_boosts.insert(crate::instruction::Stat::SpecialAttack, 1); }
                    "apicotberry" => { stat_boosts.insert(crate::instruction::Stat::SpecialDefense, 1); }
                    _ => {}
                }
                
                instruction_list.push(Instruction::BoostStats(crate::instruction::BoostStatsInstruction {
                    target_position: position,
                    stat_boosts,
                }));
