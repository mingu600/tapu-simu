...
impl RandomTeamLoader {
    ...
    pub fn load_teams(&mut self, format: &BattleFormat) -> Result<(), String> {
        ...
        if !Path::new(&file_path).exists() {
            return Err(format!("Team file not found: {}. Run team generation first.", file_path));
        }
    ...
    pub fn get_random_team(&mut self, format: &BattleFormat) -> Result<RandomTeam, String> {
        ...
        let format_key = self.format_to_file_key(format);
        let teams = self.teams_cache.get(&format_key)
        ...
        let team = teams.choose(&mut self.rng)
    ...
    pub fn get_random_teams(&mut self, format: &BattleFormat, count: usize) -> Result<Vec<RandomTeam>, String> {
...
impl RandomPokemonSet {
    ...
    pub fn get_tera_type(&self) -> Option<PokemonType> {
        self.tera_type.as_ref().and_then(|t| {
            match t.as_str() {
    ...
    /// Get EVs with default values
    pub fn get_evs(&self) -> Stats {
        match &self.evs {
            Some(evs) => evs.to_stats(85), // Random battles typically use 85 EVs
            None => Stats {
                hp: 85,
                attack: 85,
                defense: 85,
    ...
    /// Get IVs with default values
    pub fn get_ivs(&self) -> Stats {
        match &self.ivs {
            Some(ivs) => ivs.to_stats(31), // Default to perfect IVs
            None => Stats {
                hp: 31,
                attack: 31,
                defense: 31,
    ...
    pub fn to_battle_pokemon(&self, move_factory: &crate::data::ps_move_factory::PSMoveFactory, pokemon_factory: &crate::data::ps_pokemon_factory::PSPokemonFactory) -> Pokemon {
        ...
        pokemon.ability = self.ability.clone().unwrap_or_else(|| {
            pokemon_factory.get_default_ability_with_fallback(&self.species)
        });
        ...
        let evs = self.get_evs();
        
        // Calculate actual stats using Pokemon formula
        // HP = floor(((2 * base + iv + floor(ev / 4)) * level) / 100) + level + 10
        // Use i32 to prevent overflow during multiplication
        let hp = (((2 * base_stats.hp as i32 + ivs.hp as i32 + evs.hp as i32 / 4) * self.level as i32) / 100) + self.level as i32 + 10;
        
        // Other stats = floor((floor(((2 * base + iv + floor(ev / 4)) * level) / 100) + 5) * nature_modifier)
        let attack = self.calculate_stat(base_stats.attack as i16, ivs.attack, evs.attack, nature.attack_modifier());
        let defense = self.calculate_stat(base_stats.defense as i16, ivs.defense, evs.defense, nature.defense_modifier());
        let special_attack = self.calculate_stat(base_stats.special_attack as i16, ivs.special_attack, evs.special_attack, nature.special_attack_modifier());
        let special_defense = self.calculate_stat(base_stats.special_defense as i16, ivs.special_defense, evs.special_defense, nature.special_defense_modifier());
        let speed = self.calculate_stat(base_stats.speed as i16, ivs.speed, evs.speed, nature.speed_modifier());
        ...
        for (index, move_name) in self.moves.iter().enumerate() {
            if index >= 4 { break; } // Pokemon can only have 4 moves
            ...
            let move_data = match move_factory.create_move(move_name) {
                ...
                None => {
                    ...
                    eprintln!("Warning: Move '{}' not found in PS data, using placeholder", move_name);
    ...
    fn calculate_stat(&self, base: i16, iv: i16, ev: i16, nature_modifier: f64) -> i16 {
        ...
        (stat as f64 * nature_modifier) as i16
    }
...
mod tests {
    ...
    fn test_format_to_file_key() {
        ...
        assert_eq!(loader.format_to_file_key(&gen9_doubles), "gen9randomdoublesbattle");
    }
