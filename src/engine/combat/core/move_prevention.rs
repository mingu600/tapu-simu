...
pub fn cannot_use_move(
    ...
    if let Some(reason) =
        check_flinch_prevention(pokemon, move_choice, move_data, battle_state, position)
    {
        return Some(reason);
    }
    ...
    if let Some(reason) =
        check_freeze_prevention(pokemon, move_choice, move_data, battle_state, position)
    {
        return Some(reason);
    }
    ...
    if let Some(reason) =
        check_other_prevention_effects(pokemon, move_choice, move_data, battle_state, position)
    {
        return Some(reason);
    }
    ...
    match prevention {
        ...
        MovePreventionReason::Sleep { wake_up_chance } => {
            ...
            if wake_up_chance > 0.0 {
                let wake_up_instructions =
                    vec![BattleInstruction::Status(StatusInstruction::Remove {
                        target: position,
        ...
        MovePreventionReason::Confusion { self_damage } => {
            ...
            let success_instructions = vec![BattleInstruction::Status(
...
pub fn apply_flinch_effect(
    ...
    if let (Some(user_pokemon), Some(target_pokemon)) = (user, target) {
        ...
        let target_speed =
            target_pokemon.get_effective_stat(crate::core::instructions::Stat::Speed);
