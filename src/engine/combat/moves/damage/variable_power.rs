    ...
    if let (Some(user), Some(target)) = (
        ...
        if user_speed > 0 {
            ...
            let modified_move_data = MoveData {
                base_power: calculated_power.round() as u16,
                ..move_data.clone()
            };
            
            return crate::engine::combat::moves::apply_generic_effects(state, &modified_move_data, user_position, target_positions, generation, branch_on_damage);
        }
    }
...
pub fn apply_reversal(
    ...
    if let Some(user) = state.get_pokemon_at_position(user_position) {
        ...
        return apply_power_modifier_move(state, move_data, user_position, target_positions, generation, power_multiplier, branch_on_damage);
    }
...
pub fn apply_avalanche(
    ...
    let generic_instructions = crate::engine::combat::moves::apply_generic_effects(state, &modified_move_data, user_position, target_positions, generation, branch_on_damage);
...
pub fn apply_fishious_rend(
    ...
    apply_boltbeak(state, move_data, user_position, target_positions, generation, context, branch_on_damage)
}
    ...
    if let Some(user_pokemon) = state.get_pokemon_at_position(user_position) {
        for &target_position in target_positions {
            if let Some(target_pokemon) = state.get_pokemon_at_position(target_position) {
                ...
                let target_instructions = apply_generic_effects(state, &modified_move_data, user_position, target_positions, generation, branch_on_damage);
...
pub fn apply_waterspout(
    ...
    apply_eruption(state, move_data, user_position, target_positions, generation, branch_on_damage)
}
...
fn apply_power_modifier_move(
    ...
    crate::engine::combat::moves::apply_generic_effects(state, &modified_move, user_position, target_positions, generation, branch_on_damage)
}
...
pub fn apply_low_kick(
    ...
    apply_grass_knot(state, move_data, user_position, target_positions, generation, branch_on_damage)
}
...
pub fn apply_heavy_slam(
    ...
    apply_heat_crash(state, move_data, user_position, target_positions, generation, branch_on_damage)
}
...
pub fn apply_barb_barrage(
    ...
    for &target_position in target_positions {
        if let Some(target) = state.get_pokemon_at_position(target_position) {
            ...
            let mut modified_move_data = move_data.clone();
            if target.status == PokemonStatus::Poison || target.status == PokemonStatus::BadlyPoisoned {
...
pub fn apply_electro_drift(
    ...
    apply_collision_course(state, move_data, user_position, target_positions, generation, branch_on_damage)
}
    ...
    for &target_position in target_positions {
        if let Some(target) = state.get_pokemon_at_position(target_position) {
            if target.item.is_none() {
                ...
                let target_instructions = apply_generic_effects(state, move_data, user_position, target_positions, generation, branch_on_damage);
                instructions.extend(target_instructions);
            }
        }
    }
...
pub fn apply_power_trip(
    ...
    apply_stored_power(state, move_data, user_position, target_positions, generation, branch_on_damage)
}
...
pub fn apply_upper_hand(
    ...
    let mut move_instructions = crate::engine::combat::moves::apply_generic_effects(state, &modified_move_data, user_position, target_positions, generation, branch_on_damage);
    ...
    for &target_pos in target_positions {
        if let Some(opponent_info) = context.opponent_moves.get(&target_pos) {
            ...
            if let Some(repo_move_data) = repository.moves.find_by_name(&opponent_info.move_name) {
                ...
                return crate::engine::combat::moves::apply_generic_effects(state, &enhanced_move, user_position, target_positions, generation, branch_on_damage);
            }
        }
    }
