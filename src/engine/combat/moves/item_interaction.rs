...
pub fn apply_fling(
    ...
    if let Some(user) = state.get_pokemon_at_position(user_position) {
        if let Some(item) = &user.item {
            ...
            let can_be_flung = repository.can_item_be_flung(item);
...
fn get_fling_power(item: &str, repository: &Repository) -> u8 {
    repository.get_item_fling_power(item)
        .unwrap_or_else(|| {
            // If item not found in repository, we should error rather than fallback
            panic!("Item '{}' not found in repository. Ensure Pokemon data is complete.", item)
        })
}
