...
mod tests {
    ...
    fn test_generation_numbers() {
        assert_eq!(Generation::Gen1.number(), 1);
        assert_eq!(Generation::Gen9.number(), 9);
        assert_eq!(Generation::from_number(5), Some(Generation::Gen5));
        assert_eq!(Generation::from_number(10), None);
    }

    #[test]
    fn test_critical_hit_changes() {
        let gen5_mechanics = Generation::Gen5.get_mechanics();
        let gen6_mechanics = Generation::Gen6.get_mechanics();
        
        assert_eq!(gen5_mechanics.get_critical_multiplier(), 2.0);
        assert_eq!(gen6_mechanics.get_critical_multiplier(), 1.5);
    }

    #[test]
    fn test_feature_availability() {
        let gen3_mechanics = Generation::Gen3.get_mechanics();
        let gen4_mechanics = Generation::Gen4.get_mechanics();
        
        assert!(!gen3_mechanics.has_feature(GenerationFeature::PhysicalSpecialSplit));
        assert!(gen4_mechanics.has_feature(GenerationFeature::PhysicalSpecialSplit));
        
        assert!(gen3_mechanics.has_feature(GenerationFeature::Abilities));
        assert!(gen4_mechanics.has_feature(GenerationFeature::Abilities));
    }

    #[test]
    fn test_type_effectiveness_overrides() {
        let gen1_mechanics = Generation::Gen1.get_mechanics();
        let gen2_mechanics = Generation::Gen2.get_mechanics();
        
        assert_eq!(gen1_mechanics.get_type_effectiveness_override("ghost", "psychic"), Some(0.0));
        assert_eq!(gen2_mechanics.get_type_effectiveness_override("ghost", "psychic"), Some(2.0));
    }

    #[test]
    fn test_terrain_multipliers() {
        let gen6_mechanics = Generation::Gen6.get_mechanics();
        let gen7_mechanics = Generation::Gen7.get_mechanics();
        let gen8_mechanics = Generation::Gen8.get_mechanics();
        
        assert_eq!(gen6_mechanics.get_terrain_multiplier(), 1.0); // No terrain
        assert_eq!(gen7_mechanics.get_terrain_multiplier(), 1.5); // Terrain introduced
        assert_eq!(gen8_mechanics.get_terrain_multiplier(), 1.3); // Terrain nerfed
    }
}