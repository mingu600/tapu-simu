...
use crate::state::{Pokemon, State, MoveCategory, Move};
use crate::battle_format::{BattleFormat, BattlePosition, SideReference, FormatType};
use crate::data::ps_loader::PSDataRepository;
use crate::engine::damage_calc::calculate_damage;
use crate::engine::instruction_generator::GenerationXInstructionGenerator;
use crate::data::types::EngineMoveData;
use crate::move_choice::{MoveChoice, MoveIndex};
use crate::instruction::{Instruction, StateInstructions, PokemonStatus, Stat};
...
impl TestFramework {
    ...
    pub fn test_screen_effects(
        ...
        attacker_species: &str,
        defender_species: &str,
        screen: crate::instruction::SideCondition,
        move_name: &str,
    ) -> Result<f32, Box<dyn std::error::Error>> {
        let attacker = self.create_pokemon_from_ps_data(attacker_species, None, Some(50))?;
        let defender = self.create_pokemon_from_ps_data(defender_species, None, Some(50))?;
