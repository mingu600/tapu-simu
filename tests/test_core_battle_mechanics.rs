...
use tapu_simu::engine::type_effectiveness::{TypeChart, PokemonType};
...
fn test_critical_hit_damage_increase() {
    ...
    let normal_damage = tapu_simu::engine::damage_calc::calculate_damage(
        &state, &attacker, &defender, &move_data, false, 1.0
    );
    let crit_damage = tapu_simu::engine::damage_calc::calculate_damage(
...
fn test_generation_specific_critical_multipliers() {
    ...
    let gen5_state = State::new(BattleFormat::new(
        "Gen 5 OU".to_string(),
        Generation::Gen5,
        tapu_simu::battle_format::FormatType::Singles,
    ));
    ...
    let gen5_normal = tapu_simu::engine::damage_calc::calculate_damage(
        &gen5_state, &attacker, &defender, &move_data, false, 1.0
    );
    let gen5_crit = tapu_simu::engine::damage_calc::calculate_damage(
        &gen5_state, &attacker, &defender, &move_data, true, 1.0
    );
    
    let gen9_normal = tapu_simu::engine::damage_calc::calculate_damage(
        &gen9_state, &attacker, &defender, &move_data, false, 1.0
    );
    let gen9_crit = tapu_simu::engine::damage_calc::calculate_damage(
...
fn test_spread_move_damage_reduction() {
    ...
    let doubles_state = State::new(BattleFormat::new(
        ...
        Generation::Gen9,
        tapu_simu::battle_format::FormatType::Doubles,
    ));
    
    // Simulate hitting multiple targets by using calculate_damage_with_targets directly
    let doubles_damage = tapu_simu::engine::damage_calc::calculate_damage_with_targets(
